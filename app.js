...
/* ---------------- MONÉTISATION (Payhip) ----------------
   ...
   La clé secrète Payhip n'est plus dans ce fichier : elle vit dans le
   Cloudflare Worker (variable d'environnement chiffrée), qui fait la
   vérification à la place du navigateur. Voir worker.js + README.
------------------------------------------------------- */
const MONETIZATION_ENABLED = true;
// Plus de clé secrète ici : la vérification passe par un Cloudflare Worker
// qui garde la clé Payhip côté serveur (voir worker.js fourni séparément).
const CONTENT_WORKER_URL = "https://payhipverify.ceargenti.workers.dev/unlock";
const APP_ID = "go4e";
const PAYHIP_PURCHASE_URL = "https://payhip.com/b/X9DZs";
...
function defaultState(){
  return {
    ...
    lockedQuestions: null,   // questions jours 2-10, mises en cache localement après déblocage (pour usage hors-ligne)
  };
}
let STATE = loadState();
if(STATE.unlocked && Array.isArray(STATE.lockedQuestions) && STATE.lockedQuestions.length){
  // Contenu premium déjà téléchargé sur cet appareil lors d'un déblocage précédent :
  // on le réintègre localement, sans consommer d'activation supplémentaire.
  QUESTIONS.push(...STATE.lockedQuestions);
}
...
async function tryUnlock(){
  ...
  btn.disabled = true;
  try{
    const res = await fetch(`${CONTENT_WORKER_URL}?app=${encodeURIComponent(APP_ID)}&license_key=${encodeURIComponent(key)}`);
    const json = await res.json().catch(()=>null);
    if(json && json.enabled && Array.isArray(json.questions)){
      QUESTIONS.push(...json.questions);
      STATE.unlocked = true;
      STATE.lockedQuestions = json.questions; // mis en cache pour un usage hors-ligne futur
      saveState();
    ...
    }else if(json && json.reason === "activation_limit_reached"){
      msgEl.textContent = "❌ Ce code a déjà été utilisé sur le nombre maximum d'appareils autorisés.";
      msgEl.className = "unlock-msg err";
    }else{
